...
| `organization-admin` | Administration/list | organization members/invites/compliance/Documentation operations; owner/admin/editor/viewer | `f125272` | `c4141a1` | 0 | `accept` — review A | `accept` — review B | focused organization-admin 13/13; full web 492/492 serial; web typecheck/lint/build/diff; authenticated owner/viewer members/compliance browser matrix; native Page zoom 200% owner/viewer members, compliance, Documentation Operations, error/retry, and permission checks in evidence commit `48ac394`; axe 0 violations on members/permission states and the existing metric contrast probes only on compliance/Documentation Operations; no page errors | `agent_accepted_pending_human` | shared portal navigation clipping remains out of scope; native 200% metrics retain the known contrast-background/incomplete text-length probes; browser zoom controls remain a broader matrix item; no pre-change compliance desktop screenshot |
| `projects-workspace` | Dense library/workspace | project library/workspace/create/archive/unauthenticated denied; owner/admin/editor/viewer | `c6ee819` | `e94d6a9` | 3 | `accept` — prior cycles; `incomplete` — workspace state follow-up; `accept` — bounded legacy fallback continuation | `accept` — prior cycles; `needs_human_surface` — workspace state follow-up; `accept` — bounded legacy fallback continuation | Prior ProjectListPage/App/shell evidence remains valid; workspace component 9/9; adjacent Project list/shell 21/21; native Page zoom 200% owner active/archived/create/error states and viewer active/archived plus valid `/projects/:projectId` redirect evidence in `f167ee5`; the seeded valid route follows `LegacyProjectRedirect` to the Project Version workspace with axe 0/0; the anonymous fallback continuation `879a1bd` now has a named heading, `role=status`/`role=alert`, and desktop/390px/native-200% axe-clean evidence | `needs_human_surface` | `ProjectWorkspacePage` is still not mounted by the normal App route; the valid seeded redirect is evidence of current shipped routing, not acceptance of the candidate component; the route-neutral fallback does not resolve route ownership; browser zoom, P2-001, and broader 26.6 matrix remain separate; prior `aa6f892`/`3a8fad4` evidence remains linked |
| `project-versions` | Context/admin/timeline | Project Version context/settings/create/reorder/default/archive/restore/activity/Carry Forward | `d34eafe` | `c93ca11` + `2eb2ce8` follow-up | 0 | `accept` — review A | `accept` — review B | focused Project Version 12/12; full web 95 files / 509 tests; web typecheck/lint/build; CSS-token check 130/123; authenticated owner/viewer active/non-default/archived browser matrix; native Page zoom 200% owner/viewer Main/Summer release workspace context recheck, 390px owner reflow, axe 0/0, no overflow, and final GET-only request paths in follow-up evidence; the narrow identity now stacks project name, selected Project Version, and status badge without breaking the selected name | `agent_accepted_pending_human` | shared portal navigation clipping remains out of scope; the prior native-200% Project Version context split is resolved by `2eb2ce8`; browser zoom controls remain a broader matrix item; existing settings textarea probe; P2-010 is resolved in `59fd07f`; broader raw CSS cleanup remains separate |
| `capture-portal` | Library/workbench | Capture library/create/detail/assets/events/upload/retry/final/read-only | `e37df02` | `f4a6010` | 0 | `accept` — review A | `accept` — review B | focused Capture portal 58/58; full web 95 files / 509 tests; web check-types/lint/build; authenticated owner/viewer list/create/detail browser matrix; desktop/narrow axe 0 violations; native Page zoom 200% owner/viewer list, lifecycle/detail, error/retry, keyboard, and request checks in evidence commit `70b4259`; shared Project Version context reflow follow-up `2eb2ce8` rechecked owner/viewer contexts at native 200% and owner at 390px; no new post-restart browser errors or failed local requests | `agent_accepted_pending_human` | shared portal navigation clipping remains out of scope; the earlier native-200% context split is resolved by `2eb2ce8`; browser zoom controls remain a broader matrix item; populated Event/Asset browser records unavailable in disposable fixture and remain component-test coverage; create-form textarea has one incomplete contrast-background probe; P2-010 is resolved in `59fd07f`; broader raw CSS cleanup remains separate |
...
| `demos-internal` | Library/direct-manipulation workbench | Demo library/editor/scenes/hotspots/preview/Revisions/publishing | `d638112` | `e97647e` | 0 | `accept` — review A | `accept` — review B | focused Interactive Demo 39/39; full web 95 files / 509 tests; web typecheck/lint/build/Prettier; authenticated active/empty/archived/viewer/preview/Revisions browser matrix; pointer/keyboard/zoom/reduced-motion; native 200% pointer resize and move geometry follow-up with reload-to-100%-percent local-state reset, axe 0 violations plus the known three-textarea incomplete probe, no overflow, and GET-only final paths | `agent_accepted_pending_human` | one existing incomplete three-textarea contrast probe on active editor; native direct-manipulation evidence is supplemental and does not close the complete 26.6 matrix; shared shell/public access remain separate |
...
| `extension-installation` | Setup utility | extension check/auth/error/ready/download/update/remove | `7893091` | `1058dbd` | 0 | `accept` — review A | `accept` — review B | focused extension installation 22/22 plus TDD auth-error heading follow-up; full web 95 files / 508 tests; web check-types/lint/build; authenticated ready/download-error/auth-check-error browser states at native Chrome Page zoom 200% (`dpr=2`, 525px CSS viewport), axe 0/0 after `fc8071c`, no overflow, reduced-motion and keyboard checks; ZIP download returned 200 | `agent_accepted_pending_human` | `fc8071c` adds the missing level-one heading to the auth-check error state; no new blind-review cycle is claimed for this bounded accessibility follow-up; unpacked extension load and pin-to-toolbar details remain separately evidenced; actual toolbar-icon popup activation remains unclaimed; shared portal-shell clipping and broader zoom matrix remain separate |
...
| `contributor-docs` | Separate documentation site | `apps/docs` landing/content; public | `de37b5e` | `ae37ba6` | 0 | `accept` — review A | `accept` — review B | focused docs page/content 10/10; docs check-types/lint/build; local desktop/narrow browser matrix; native Chrome Page zoom 200% (`dpr=2`, 525px CSS viewport, 4,798px document height), all four image elements loaded, axe 0/0, reduced-motion and keyboard checks, no horizontal overflow; screenshots `147-continuation-contributor-docs-*.png` | `agent_accepted_pending_human` | external source links were not followed; native 200% is bounded continuation evidence only; customer Product Documentation remains a separate app/web surface |
...
| `P2-006` | P2 | workspace composition | `projects-workspace` active/archived/create/unauthenticated denied, 1440px and 390px | surface preflight; baseline showed a sparse desktop field, weak filter grouping, loose card hierarchy, denied state without a level-one heading, and transient loading/error states without page/status semantics | implementer | 3 | needs_human_surface | `e94d6a9` | ProjectWorkspacePage 9/9; adjacent Project list/shell 21/21; native-200% Project list active/archived/error and valid seeded project redirect evidence is recorded in `f167ee5`; the valid route resolves to Project Version workspace, not `ProjectWorkspacePage`; the earlier `/projects/project_1` fallback still reports the missing-h1 finding; both final-cycle reviews record incomplete/needs-human | route ownership is unresolved and the new valid redirect evidence does not authorize wiring the candidate component; do not change route semantics without human direction; browser zoom and broader 26.6 matrix remain separate |
...
| `P2-015` | P2 | extension installation composition / accessibility | Authenticated `/extension` ready state; desktop 1440px and narrow 390px | surface preflight; baseline had a Capture-tools contrast violation and an incomplete gradient-background probe over the download card | implementer | 0 | accepted_pending_human | `1058dbd` + `fc8071c` follow-up | named installation workspace, tokenized hierarchy and cards, authenticated desktop/narrow browser evidence, native-200% ready/download-error/auth-check-error evidence, auth-check error state now has a level-one heading, axe 0/0, keyboard/reduced-motion, focused 22/22 plus follow-up test, full web 95 files / 508 tests, check-types/lint/build, and both original reviews pass | installed toolbar/permission path is unavailable in this runner; no new blind-review cycle claimed for the bounded heading follow-up; shared shell clipping and browser zoom remain separate from resolved P2-010 |
...
| `P2-019` | P2 | accessibility landmark | authenticated Documentation Review Inbox, owner/viewer, native Chrome Page zoom 200% | native browser audit found the route component rendered a nested/duplicate unnamed `main` landmark inside the portal shell | implementer | 0 | accepted_pending_human | `ab10940` | TDD red/green focused contract; full web 95 files / 507 tests; owner and viewer native-200% recheck axe 0/0 with one shell-owned `main` and a labeled inbox region; no mutation submitted | broader responsive/state/keyboard matrix, shared shell, and human review remain separate |
...
| `P2-002` | P2 | mobile composition | shared authenticated portal shell / 1440px, 390px, and 320px | shared-shell-mobile preflight; PortalAppShell navigation measured 837px wide inside a 390px viewport | implementer | 0 | accepted_pending_human | `8b45a4b` + `2eb2ce8` follow-up | focused shell 6/6; full web 95 files / 509 tests; web typecheck/lint/build; 1440px/390px/320px browser evidence; focused navigation axe 0/0; Project Version native-200% owner/viewer follow-up and 390px owner context reflow observed no document overflow and kept the Project Version identity readable beside Manage Versions | actual browser zoom control was unavailable to the original candidate; unauthenticated `/projects` is the truthful browser state and authenticated role contexts remain component-test coverage; the previously observed native-200% context split is resolved by the bounded follow-up; P2-001 and the broader matrix remain separate |
...
| `879a1bd` | `projects-workspace` | A — bounded legacy fallback visual/interaction continuation | `accept` | `docs/ui/147-project-workspace-legacy-fallback-review-a.md` | none | route ownership and `ProjectWorkspacePage` runtime mounting remain separate human decisions | App/workspace focused 31/31; desktop, 390px, and native Page zoom 200% evidence; axe 0/0; no client/scroll overflow; expected local 401 boundary | accepted pending human |
| `879a1bd` | `projects-workspace` | B — bounded legacy fallback product/a11y/engineering continuation | `accept` | `docs/ui/147-project-workspace-legacy-fallback-review-b.md` | none | no API/domain/permission/tenant/immutability/public-link/mutation change; broader matrix and route ownership remain separate | named main/heading, status/alert semantics, 31/31 focused tests, native-200% dpr 2 / 525px evidence, axe 0/0, expected GET/401 network boundary, no page errors | accepted pending human |
...
| `projects-workspace` | route-neutral legacy redirect fallback / desktop 1440×900 | local anonymous `project_1`; no Project data or credentials | agent-browser / Chromium; runner API 3002/web 3020 | historical fallback had no level-one heading or status/error role | existing route now renders `Project not found` h1 and `Project was not found.` alert; document/client/scroll width 1440px; axe 0/0; `docs/ui/147-project-workspace-legacy-fallback-after-desktop-1440.png` | no | reduced-motion enabled; URL remained `/projects/project_1`; expected Project 401; console only Vite/React development notices; no page errors | presentation-only correction; successful redirect and API boundary unchanged | `879a1bd` / 2026-08-07 |
| `projects-workspace` | route-neutral legacy redirect fallback / narrow 390×844 | same local anonymous response | agent-browser / Chromium; runner web 3020/API 3002 | same historical fallback hierarchy gap | `Project not found` h1 and alert remain visible; client/scroll/body width 390px; axe 0/0; Tab leaves body because no recovery action is invented; `docs/ui/147-project-workspace-legacy-fallback-after-mobile-390.png` | no | reduced-motion enabled; no page errors; expected Project 401; no mutation | narrow fallback remains bounded without changing terminal not-found behavior | `879a1bd` / 2026-08-07 |
| `projects-workspace` | route-neutral legacy redirect fallback / native Chrome Page zoom 200% | same local anonymous response | agent-browser / Chromium; Page zoom set through `chrome://settings/appearance` | historical fallback hierarchy gap | `dpr=2`, CSS viewport 525×450, client/scroll/body width 525px; `Project not found` h1 and alert; axe 0/0; `docs/ui/147-project-workspace-legacy-fallback-after-native-200-525px.png` | no | reduced-motion enabled; expected Project 401; console/page errors clean apart from development notices; zoom restored to 100% and browser closed | native zoom proof is bounded to this fallback; full 26.6 matrix remains open | `879a1bd` / 2026-08-07 |
...
| `documentation-public` | anonymous valid Publication / tablet 1024×768 | disposable Documentation fixture; `plan132-public` Install Publication | agent-browser / Chromium; runner API 3022/web 3020 | prior valid reader evidence at desktop/narrow | Install reader rendered one main, one h1, 13 focusable controls, client/scroll width 1024px; axe 0/0; `docs/ui/147-public-documentation-tablet-1024.png` | no | reduced-motion enabled; semantic snapshot inspected; no horizontal overflow | public Documentation request returned 200; console only expected Vite/React development notices; no page errors | tablet evidence supplements existing candidate evidence without changing Publication/content/link behavior | `88750fb` / 2026-08-07 |
| `documentation-public` | anonymous valid Publication / public narrow 320×844 | same disposable Documentation fixture | agent-browser / Chromium; runner API 3022/web 3020 | prior narrow evidence at 390px | Install reader retained one main and one h1; client/scroll/body width 320px; axe 0/0; first Tab focused `Skip to content`; `docs/ui/147-public-documentation-narrow-320.png` | no | reduced-motion enabled; keyboard skip-link path verified; no page overflow | public Documentation request returned 200; no page errors; no target request or mutation | 320px supplements the accepted 390px reader state and keeps the mobile navigation boundary truthful | `88750fb` / 2026-08-07 |
...
| `demos-public` | anonymous valid reader / tablet 1024×768 | disposable Interactive Demo fixture; `plan128-public` active Publication | agent-browser / Chromium; runner API 3022/web 3020 | prior valid reader evidence at desktop/narrow | active reader rendered one main, h1 `Plan 128 active Interactive Demo`, public Version selector, and Published start; client/scroll/body width 1024px; axe 0/0; `docs/ui/147-public-demo-tablet-1024.png` | no | reduced-motion enabled; semantic snapshot inspected; no horizontal overflow | public Demo request returned 200; console only expected Vite/React development notices; no page errors | tablet evidence supplements the existing reader without changing immutable playback or public-link behavior | `88750fb` / 2026-08-07 |
| `demos-public` | anonymous valid reader / public narrow 320×844 | same disposable Interactive Demo fixture | agent-browser / Chromium | prior narrow evidence at 390px | active reader retained one main and h1; client/scroll/body width 320px; axe 0/0; first Tab reached the public Version selector; `docs/ui/147-public-demo-narrow-320.png` | no | reduced-motion enabled; keyboard entry reached a native control; no overflow | public Demo request returned 200; no page errors or mutation | 320px supplements the accepted 390px reader boundary; playback/access semantics remain unchanged | `88750fb` / 2026-08-07 |
| `demos-public` | anonymous valid embed / tablet 1024×768 | same disposable Interactive Demo fixture; `/d/plan128-public/embed` | agent-browser / Chromium; runner API 3022/web 3020 | prior embed evidence at desktop/narrow | embed rendered one main, h1 `Plan 128 active Interactive Demo`, client/scroll/body width 1024px; axe 0/0; `docs/ui/147-public-demo-embed-tablet-1024.png` | no | reduced-motion enabled; no horizontal overflow | embed request returned 200; console only expected Vite/React development notices; no page errors | outer embed remains a public playback frame with no authoring/admin controls | `88750fb` / 2026-08-07 |
...
| `guides-public` | anonymous valid reader / fresh tablet 1024×768 | disposable Guide fixture; `plan127-public` immutable Publication | agent-browser / Chromium; runner API 3022/web 3020 | prior valid reader evidence | fresh reader rendered one main and h1 `Plan 127 active Guide`; client/scroll/body width 1024px; axe 0/0; `docs/ui/147-public-guide-tablet-1024.png` | no | reduced-motion enabled; no overflow; semantic heading/landmark inspected | public Guide request returned 200; console only expected development notices; no page errors | fresh tablet evidence corroborates the earlier tablet sample without changing Guide content/access semantics | `5df8696` / 2026-08-07 |
...
| `guides-public` | anonymous valid reader / public narrow 320×844 | disposable Guide fixture; `plan127-public` immutable Publication | agent-browser / Chromium; runner API 3022/web 3020 | prior valid reader evidence at 390px and tablet | valid reader rendered one main and h1 `Plan 127 active Guide`; client/scroll/body width 320px; axe 0/0; `docs/ui/147-public-guide-narrow-320.png` | no | reduced-motion enabled; first Tab left focus on body because the static Guide has no focusable controls; no overflow | public Guide request returned 200; console only expected development notices; no page errors | 320px supplements the existing 390px reader evidence without changing Guide Revision/access semantics | `5df8696` / 2026-08-07 |
| `guides-public` | anonymous valid embed / tablet 1024×768 | same disposable Guide fixture; `/p/plan127-public/embed` | agent-browser / Chromium; runner API 3022/web 3020 | prior embed evidence at 390px | embed rendered one main and h1 `Plan 127 active Guide`; client/scroll/body width 1024px; axe 0/0; `docs/ui/147-public-guide-embed-tablet-1024.png` | no | reduced-motion enabled; no overflow | public Guide embed request returned 200; no page errors; no target request or mutation | embed remains a public reader frame with no authoring controls | `5df8696` / 2026-08-07 |
...
| `native-zoom-guide-hardening` | anonymous valid Guide reader / native Chrome Page zoom 200% | fresh synthetic Plan 127 public Publication; no mutation | separate headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | valid Guide evidence had prior desktop/narrow coverage | `Plan 127 active Guide` rendered with one main and `Step 1`; `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; screenshot `docs/ui/147-continuation-public-guide-zoom-200.png` | no | no visible overflow; reduced motion true; Tab reached the body because this immutable reader exposes no skip-link/control target | public Publication and asset requests returned 200; no page errors; no mutation | valid reader remains immutable and public-link-owned; the lack of a skip target is recorded as bounded keyboard coverage, not a full keyboard-matrix claim | `2922665` / 2026-08-07 |
| `native-zoom-guide-hardening` | anonymous Guide embed / native Chrome Page zoom 200% | same synthetic Plan 127 public Publication; no mutation | separate headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | embed evidence had prior desktop coverage | embedded immutable Guide rendered one main and the same h1 at `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; screenshot `docs/ui/147-continuation-public-guide-embed-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | embed requests completed successfully; no authoring or private metadata exposed | embed remains a restrained public playback frame; no mutation or target request was submitted | `2922665` / 2026-08-07 |
| `native-zoom-guide-hardening` | anonymous Guide password/access boundaries / native Chrome Page zoom 200% | fresh synthetic Plan 127 password, restricted, expired, and revoked Publish Links; safe invalid-password probe; no secret recorded | separate headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | access evidence had prior desktop/narrow coverage | password invalid retry retained `Password is invalid.`; restricted/expired/revoked retained truthful h1 states; widths stayed within 525px and axe 0/0; screenshot `docs/ui/147-continuation-public-guide-password-invalid-zoom-200.png` | no | no page errors; expected public-link statuses were 401/400, 403, 410, and 404; no secret recorded | only a safe invalid password was submitted; no successful password or link mutation was attempted | supplements, but does not replace, the full Guide public/auth/embed matrix | `2922665` / 2026-08-07 |
| `native-zoom-guide-auth-boundary` | authenticated Guide editor / native Chrome Page zoom 200% | fresh synthetic Plan 127 editor; same-origin real `/login`; active Guide; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | authenticated Guide editor evidence had prior desktop/narrow coverage | active Guide editor rendered with `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`; screenshot `docs/ui/147-continuation-auth-guide-editor-zoom-200.png` | no | axe 0 violations / 1 known incomplete two-textarea contrast-background probe; no visible overflow; reduced motion true | login and Guide reads completed; no save/archive/publish/delete action submitted; console contained expected development notices | known incomplete probe is recorded rather than presented as axe 0/0; Guide mutation semantics remain unchanged | `2922665` / 2026-08-07 |
| `native-zoom-guide-auth-boundary` | authenticated Guide viewer / native Chrome Page zoom 200% | fresh synthetic Plan 127 viewer; same Guide route; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | viewer Guide evidence had prior desktop/narrow coverage | Guide rendered `Read only` with `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; no Save/Archive/Publish/Delete controls; screenshot `docs/ui/147-continuation-auth-guide-viewer-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | viewer login and reads completed; server authorization remains authoritative; no mutation submitted | UI absence is supplemental evidence only and does not infer broader permission-matrix completion | `2922665` / 2026-08-07 |
| `native-zoom-shared-foundation` | local Design System review gallery / native Chrome Page zoom 200% | synthetic Child 121 patterns only; no authentication, private API calls, customer data, or mutation | headed `agent-browser` Chromium; runner web 3020; reduced motion | gallery had prior desktop/narrow evidence but no native 200% proof | one main and `Design system review` h1; `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`; artifact table client/scroll width `484px`; screenshot `docs/ui/147-continuation-design-system-zoom-200.png` | no | axe 0 violations / 1 known synthetic contrast-background incomplete probe; no visible document overflow; keyboard reached Retry and the artifact-table region; reduced motion true | only expected Vite/React development notices; no API or mutation requests | synthetic gallery patterns do not claim production-route or full shared-consumer 200% closure | `c84f897` / 2026-08-07 |
| `native-zoom-entry-fallback` | public `/login` entry / native Chrome Page zoom 200% | no authentication submitted; no credentials recorded | headed `agent-browser` Chromium; runner web 3020; reduced motion | login had prior desktop/narrow evidence but no native 200% proof | `Sign in` h1 and one main; `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`; screenshot `docs/ui/147-continuation-login-zoom-200.png` | no | axe 0/0; reduced motion true; Tab sequence reached the Email field; no visible overflow | only local Vite/React development notices; no login request was submitted | this is entry composition evidence, not successful auth proof (real auth is recorded in other rows) | `c84f897` / 2026-08-07 |
| `native-zoom-entry-fallback` | unsupported route `/unknown-plan147` / native Chrome Page zoom 200% | anonymous local route; no data or mutation | headed `agent-browser` Chromium; runner web 3020; reduced motion | fallback had prior desktop/narrow evidence but no native 200% proof | truthful `Page not found` h1, one main, safe `Open Projects` and `Sign in` links; `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `525px`; screenshot `docs/ui/147-continuation-unknown-fallback-zoom-200.png` | no | axe 0/0; reduced motion true; no visible overflow or page errors | only local Vite/React development notices; no API or mutation request | recovery links preserve existing route semantics; no shell/domain behavior changed | `c84f897` / 2026-08-07 |
| `native-zoom-demo-internal` | authenticated Interactive Demo editor / native Chrome Page zoom 200% | fresh synthetic Plan 128 editor; active 12-Scene Working Draft; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | editor evidence had prior desktop/narrow coverage but no native 200% proof | active editor rendered one main and `Plan 128 active Interactive Demo` h1; `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`; keyboard Enter moved Scene 1→Scene 2; screenshot `docs/ui/147-continuation-auth-demo-editor-zoom-200.png` | no | axe 0 violations / 1 known three-textarea contrast-background incomplete probe; no visible overflow; reduced motion true | editor login and reads completed; scene selection changed client state only; no Save/Archive/Publish/Delete/mutation action submitted | the incomplete probe is recorded rather than presented as axe 0/0; direct-manipulation breadth remains separately limited | `2354803` / 2026-08-07 |
| `native-zoom-demo-internal` | authenticated Interactive Demo Working Draft preview / native Chrome Page zoom 200% | same synthetic Plan 128 active Working Draft; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | preview evidence had prior desktop/narrow coverage | preview rendered one main, h1, Scene 1 of 12, and immutable playback content at `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; screenshot `docs/ui/147-continuation-auth-demo-preview-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | no publish/save/restore action submitted; preview stayed a read-only Working Draft projection | preview is bounded to the synthetic editor session and does not claim public-link behavior | `2354803` / 2026-08-07 |
| `native-zoom-demo-internal` | authenticated Interactive Demo Revision history / native Chrome Page zoom 200% | same synthetic Plan 128 immutable Revisions; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | Revision evidence had prior desktop/narrow coverage | `Revisions` h1, immutable history label, Revision 1/2 entries, and checkpoint/restore affordances rendered at `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; screenshot `docs/ui/147-continuation-auth-demo-revisions-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | Restore/Create checkpoint controls were observed but not activated; no immutable history mutation submitted | control presence does not imply an action was performed; Revision semantics remain API-owned | `2354803` / 2026-08-07 |
| `native-zoom-demo-internal` | authenticated Interactive Demo viewer active state / native Chrome Page zoom 200% | fresh synthetic Plan 128 viewer; active Demo; no mutation | separate headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | viewer evidence had prior desktop/narrow coverage | active Demo rendered one main at `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; no Save/Archive/Publish/Delete/Add Scene/Add hotspot controls; screenshot `docs/ui/147-continuation-auth-demo-viewer-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | viewer login and reads completed; server authorization remains authoritative; no mutation submitted | UI absence is supplemental evidence only and does not infer full permission-matrix completion | `2354803` / 2026-08-07 |
| `native-zoom-demo-internal` | Interactive Demo empty and archived read-only states / native Chrome Page zoom 200% | same synthetic Plan 128 empty and archived Demos; no mutation | separate headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion | empty/archived evidence had prior desktop/narrow coverage | empty h1 `Plan 128 empty Interactive Demo` and archived h1 `Plan 128 archived Interactive Demo` each rendered one main, `Interactive demo · read only`, and `This demo has no scenes.` at `devicePixelRatio=2`, CSS viewport `525px`, document client/scroll width `517px`, axe 0/0; screenshot `docs/ui/147-continuation-auth-demo-archived-zoom-200.png` | no | no visible overflow; reduced motion true; no page errors | no mutation controls were submitted; archived/empty lifecycle semantics remained truthful | empty and archived proof is bounded to the synthetic fixture and does not close loading/error/denied breadth | `2354803` / 2026-08-07 |
| `native-zoom-documentation-internal` | authenticated Documentation Site owner workbench / Author, Site settings, Review, and Content task states at native Chrome Page zoom 200% | fresh synthetic Documentation fixture; real same-origin owner login; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion true | authoring evidence had prior desktop/narrow coverage but no native 200% proof for these task states | owner Site workbench rendered one shell-owned main and `Plan 132 Product Documentation`; Author, Review, and Content states stayed within `devicePixelRatio=2`, CSS viewport 525px, document client/scroll width 525px; Author/Review axe 0/0, Content retained one known overlapped-background textarea incomplete probe; scrolled screenshot `docs/ui/147-continuation-auth-documentation-author-zoom-200.png` | no | no visible horizontal overflow; reduced motion true; native controls remained present; no page errors | owner login and Documentation reads completed; no Save, navigation, review, asset, or publication mutation submitted | Content’s known incomplete probe is recorded; this continuation supplements rather than closes the full Documentation state matrix | `ab10940` / 2026-08-07 |
| `native-zoom-documentation-internal` | authenticated Documentation Page editor / native Chrome Page zoom 200% | same synthetic Documentation fixture; owner session; Install Page; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion true | Page editor evidence had prior desktop/narrow coverage but no native 200% proof | `Install` Page editor rendered one shell-owned main at `devicePixelRatio=2`, CSS viewport 525px, document client/scroll width 525px; axe 0 violations / 1 known overlapped-background textarea/comment probe; scrolled screenshot `docs/ui/147-continuation-auth-documentation-page-editor-zoom-200.png` | no | no visible horizontal overflow; reduced motion true; no Save Page, comment, import, or block mutation submitted | owner Page reads completed successfully; no final-path request failures or page errors recorded | known incomplete probes and broader editing/keyboard matrix remain separate; Page content and comment semantics were not changed by evidence | `ab10940` / 2026-08-07 |
| `native-zoom-documentation-internal` | authenticated Documentation draft preview / native Chrome Page zoom 200% | same synthetic Documentation fixture; owner and separate viewer sessions; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion true | draft preview evidence had prior desktop/narrow coverage but no native 200% proof | owner and viewer preview rendered one main, `Plan 132 Product Documentation preview`, Install/API reference content, `devicePixelRatio=2`, CSS viewport 525px, document client/scroll width 525px, axe 0/0; scrolled screenshot `docs/ui/147-continuation-auth-documentation-preview-zoom-200.png` | no | no visible horizontal overflow; reduced motion true; no publish/save action submitted | preview reads completed without final-path failures or page errors; saved-draft notice remained truthful | preview remains an internal saved-draft projection and does not claim public Publication-reader behavior | `ab10940` / 2026-08-07 |
| `native-zoom-documentation-internal` | authenticated Documentation Review Inbox / owner and viewer at native Chrome Page zoom 200% | same synthetic Documentation fixture; separate owner/viewer sessions; no mutation | separate headed `agent-browser` Chromium sessions; runner API 3022/web 3020; reduced motion true | initial audit exposed three moderate landmark violations from a nested route-owned `main` | localized repair changed the route content to a labeled region; both owner and viewer rendered one shell-owned main, `Documentation review inbox`, and 525px client/scroll width; after-fix axe 0/0; viewer notification and safe `Mark read` affordance visible; screenshot `docs/ui/147-continuation-viewer-documentation-review-inbox-zoom-200.png` | no | no visible horizontal overflow; reduced motion true; no notification was marked read; no page errors | inbox reads completed with successful local requests; the initial 3-landmark finding was fixed in `ab10940` and rechecked in both role sessions | no permission decision inferred from UI presence; broader review decision and keyboard matrix remain separate | `ab10940` / 2026-08-07 |
| `native-zoom-documentation-internal` | authenticated Documentation viewer workbench and Page editor / native Chrome Page zoom 200% | same synthetic Documentation fixture; separate viewer session; no mutation | headed `agent-browser` Chromium; runner API 3022/web 3020; reduced motion true | viewer authoring evidence had prior desktop/narrow coverage but no native 200% proof | viewer Site workbench exposed no create/save/archive/publish controls; viewer Page route exposed only saved content/export/comments, with one main and 525px client/scroll width; both routes axe 0/0 | no | no visible horizontal overflow; reduced motion true; viewer remained read-only; no mutation submitted | viewer login and reads completed; server authorization remains authoritative; no page errors recorded | UI absence is supplemental evidence only and does not infer the complete permission matrix | `ab10940` / 2026-08-07 |
...
| 2026-08-07 | `2922665` | native 200% Guide-family hardening continuation | isolated anonymous Guide reader/embed/password/access states and separate editor/viewer sessions were exercised at native Page zoom 200%; sampled widths stayed within 517–525px; public states and viewer were axe 0/0; editor retained the known two-textarea incomplete contrast probe; no mutations submitted; zoom restored to 100%, storage cleared, services stopped, and Documentation fixture reseeded | retain the unchecked broader 26.6 matrix, no-skip-target keyboard limitation for the bounded Guide reader sample, and all human/toolbar/route-ownership holds |
| 2026-08-07 | `c84f897` | native 200% shared foundation/entry/fallback continuation | local Design System gallery, `/login`, and unsupported-route fallback were exercised at native Page zoom 200%; gallery document width 517px and table width 484px; gallery retained one known incomplete contrast probe, Login and fallback were axe 0/0; keyboard reached Retry/table and Email; zoom restored to 100%, storage cleared, web service stopped | retain the unchecked broader 26.6 matrix, synthetic-gallery limitation, toolbar block, and all human route/closeout holds |
| 2026-08-07 | `2354803` | native 200% Interactive Demo internal continuation | authenticated editor, Working Draft preview, Revision history, separate viewer active state, and empty/archived lifecycle states were exercised at native Page zoom 200%; sampled documents stayed within 517px; preview, Revisions, viewer, empty, and archived were axe 0/0; editor retained the known three-textarea incomplete contrast probe; keyboard scene selection changed only client state; no mutations submitted; zoom restored to 100%, storage cleared, services stopped, and Documentation fixture reseeded | retain the unchecked broader 26.6 matrix, direct-manipulation breadth, toolbar block, and all human route/closeout holds |
| 2026-08-07 | `ab10940` | native 200% Documentation authoring/review continuation and landmark repair | owner and separate viewer sessions covered the Documentation Site workbench, Page editor, draft preview, and Review Inbox at native Page zoom 200%; sampled documents stayed within 525px; owner/viewer preview, Review Inbox, and viewer authoring/page states were axe 0/0; Content and Page editor retained only known overlapped-background textarea probes; initial Review Inbox nested-main violations were fixed test-first and rechecked; screenshots were visually inspected; no mutations submitted; zoom restored to 100%, storage cleared, services stopped | retain broader 26.6 responsive/state/keyboard matrix, P2-001 route decision, `blocked_local_for_run` toolbar status, known textarea probes, and human review; no Plan 147 completion inferred |
| 2026-08-07 | `70b4259` | native 200% Capture portal continuation | separate synthetic owner and viewer sessions covered Capture lists for active/named/archived Project Versions, completed/empty/capturing/canceled/archived details, owner authoring controls, viewer read-only controls, and an aborted-list error with Retry at native Chrome Page zoom 200%; sampled documents stayed within 525px with axe 0/0; keyboard reached the first Capture link after New Capture Session; Retry restored the seeded list; completed-detail final-path requests returned 200; screenshots were visually inspected; no mutations submitted; zoom restored to 100%, storage cleared, services stopped, and the Documentation fixture was reseeded | retain the broader 26.6 responsive/state/keyboard matrix, the shared Project Version header split visible at native 200%, populated Event/Asset fixture limitation, existing form probe, P2-001 route decision, `blocked_local_for_run` toolbar status, and human review; no new blind-review cycle or Plan 147 completion inferred |
| 2026-08-07 | `b004e44` | native 200% Project Version continuation | separate synthetic owner and viewer sessions covered Main/Summer release/Archived release workspaces, owner settings and Project Version management, empty Carry Forward, controlled Project Version list error/Retry, viewer read-only workspace and denied settings, and a keyboard path through the authenticated workspace at native Chrome Page zoom 200%; sampled documents stayed within 525px; workspace, Carry Forward, and denied settings audits were axe 0/0, while settings retained only the known overlapped-background textarea probe; final GET requests returned 200 and the only POST was synthetic login; screenshots were visually inspected; no mutation submitted; zoom restored to 100%, storage cleared, services stopped, and the Documentation fixture was reseeded | retain the broader 26.6 responsive/state/keyboard matrix, the shared Project Version context split visible at native 200%, existing settings textarea probe, P2-001 route decision, `blocked_local_for_run` toolbar status, and human review; no new blind-review cycle or Plan 147 completion inferred |

| 2026-08-07 | `48ac394` | native 200% Organization administration continuation | separate synthetic owner and viewer sessions covered Organization members/invites, compliance timeline, Documentation Operations usage/limits, owner error/Retry recovery, and viewer permission/read-only states at native Chrome Page zoom 200%; sampled documents stayed within 525px with no page errors; members and denied states were axe 0/0, while compliance and Documentation Operations retained the known metric contrast/background or short-text incomplete probes; screenshots were visually inspected; no mutation submitted; zoom restored to 100%, storage cleared, services stopped, and the Documentation fixture was reseeded | retain the broader 26.6 responsive/state/keyboard matrix, known Organization metric probes, shared shell/zoom limitation, P2-001 route decision, `blocked_local_for_run` toolbar status, and human review; no new blind-review cycle or Plan 147 completion inferred |

| 2026-08-07 | `fc8071c` + evidence closeout | native 200% extension-installation and contributor-docs continuation | authenticated synthetic owner `/extension` ready and download-error states were exercised at native Chrome Page zoom 200% (`dpr=2`, 525px CSS viewport); a forced auth-check failure exposed the missing-h1 residual, then the TDD follow-up added the `Browser extension` heading and rechecked that state at axe 0/0; ZIP download returned 200, no overflow, reduced-motion and Skip-link keyboard checks passed; public `apps/docs` was exercised at native 200% with axe 0/0, 525px CSS width, 4,798px document height, four loaded images, no overflow, and README/self-hosting keyboard path; screenshots were visually inspected; external links were not followed; docs evidence is in `147-continuation-extension-installation-*.png` and `147-continuation-contributor-docs-*.png` | retain `blocked_local_for_run` for toolbar-icon activation, the broader 26.6 matrix, separate customer Product Documentation boundary, and human review; no new blind-review cycle is claimed for this evidence continuation or the bounded heading follow-up |

| 2026-08-07 | `2eb2ce8` + evidence closeout | native 200% Project Version context reflow follow-up | TDD red/green added a responsive CSS contract for the shared Project Version context identity; the owner Main and Summer release contexts and separate viewer Main context were rechecked at native Chrome Page zoom 200% (`dpr=2`, 525px CSS viewport) with axe 0/0, no overflow, and GET-only final paths; the prior split inside `Main` is replaced by a narrow stacked identity that keeps the project name, selected Project Version, and status badge readable beside Manage Versions; the owner 390px route also remained axe 0/0 and within 390px; screenshots were visually inspected and no product mutation was submitted | retain human review, the broader 26.6 matrix, shared portal navigation clipping, existing settings textarea probe, route-ownership/P2-001 holds, and `blocked_local_for_run` toolbar activation; no new blind-review cycle is claimed for this bounded CSS follow-up |

| 2026-08-07 | `e97647e` + evidence closeout | native 200% Interactive Demo direct-manipulation continuation | the authenticated synthetic owner editor was exercised at native Chrome Page zoom 200% (`dpr=2`, 525px CSS viewport); the real resize handle reduced the selected Hotspot 1 geometry, a real pointer drag moved it, the resulting local geometry was visually inspected, and a reload restored the persisted `100%` geometry because no Save action or mutation was submitted; axe reported 0 violations with the known three-textarea incomplete probe, no horizontal overflow, no browser errors, and final paths were GET-only; evidence is in `147-continuation-auth-demo-direct-manip-zoom-200-resize.png` and `147-continuation-auth-demo-direct-manip-zoom-200-move.png` | retain the broader direct-manipulation/state matrix, known textarea probe, human review, route/public-access holds, and `blocked_local_for_run` toolbar activation; no new blind-review cycle is claimed for this evidence-only continuation |
| 2026-08-07 | `4edbcba` → current docs checkpoint | fresh broad engineering gate rerun | verification-only continuation from the clean source worktree: `pnpm -r --if-present test` passed all active workspace suites, including web 95 files/509 tests, server 127 files/553 tests, extension 19 files/140 tests, and docs 4 files/13 tests; `pnpm check-types` passed 13/13 tasks; `pnpm lint` passed 14/14 with 89 existing server warnings and zero errors; `pnpm build` passed 13/13 with the existing web chunk-size warning; `pnpm check-css-tokens` passed 130 definitions/123 consumers; `git diff --check` passed; direct commands were used because `rtk` remains unavailable | retain `agent_accepted_pending_human`, `needs_human_surface` for Project workspace/P2-001, `blocked_local_for_run` for installed-toolbar activation, the broader matrix, and human closeout; no new candidate or blind-review cycle |
| 2026-08-07 | `879a1bd` + evidence closeout | `projects-workspace` route-neutral legacy fallback continuation | TDD red/green added loading/error assertions; App/workspace focused run passed 31/31; real anonymous `/projects/project_1` kept the existing URL and expected Project 401; loading has `Opening Project` + `role=status`, failure has `Project not found` + `role=alert`; desktop 1440px, narrow 390px, and native Page zoom 200% (`dpr=2`, 525px CSS viewport) had no overflow and axe 0/0; no page errors, no mutation, services/browser cleaned up; two bounded blind reviews accept | retain `needs_human_surface` for Project workspace/P2-006 route ownership, the broader 26.6 matrix, P2-001, `blocked_local_for_run` toolbar activation, and human closeout; this continuation does not mount or accept `ProjectWorkspacePage` |
| 2026-08-07 | `879a1bd` → current docs checkpoint | final broad gates for route-neutral fallback candidate | recursive tests passed all active suites, including web 95 files/511 tests, server 127 files/553 tests, extension 19 files/140 tests, and docs 4 files/13 tests; check-types 13/13; lint 14/14 with the existing 89 server warnings and zero errors; build 13/13 with the existing web chunk-size warning; CSS tokens 130/123; diff check passed; direct commands used because `rtk` remains unavailable | retain `needs_human_surface` for Project workspace/P2-006 route ownership, P2-001, the broader 26.6 matrix, `blocked_local_for_run` toolbar activation, and human closeout; no new product semantic or route-ownership decision |
| 2026-08-07 | `88750fb` + evidence closeout | public reader viewport continuation | separate disposable Documentation and Interactive Demo fixture runs; anonymous valid readers at 1024×768 and 320×844 plus Demo embed at 1024×768 | Documentation and Demo public requests returned 200; all sampled states axe 0/0, no horizontal overflow, reduced motion enabled, Documentation Skip-link and Demo Version-selector keyboard entry checked; no mutations or target requests; Documentation fixture reseeded after Demo; five screenshots recorded | retain the broader 26.6 matrix, native-200%/toolbar limitations, P2-001 and Project workspace route-ownership decisions, and human closeout; no new candidate or blind-review cycle |
| 2026-08-07 | `5df8696` + evidence closeout | Guide public viewport continuation | separate disposable Guide fixture; valid anonymous reader at 1024×768 and 320×844 plus embed at 1024×768 | all public requests returned 200; axe 0/0, no overflow, reduced motion, one main and expected heading; static Guide had no focusable controls so first Tab remained on body; no mutations or target requests; Documentation fixture reseeded afterward | retain the broader 26.6 matrix, native-200%/toolbar limitations, P2-001 and Project workspace route-ownership decisions, and human closeout; no new candidate or blind-review cycle |
