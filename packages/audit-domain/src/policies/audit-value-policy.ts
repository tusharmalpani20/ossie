...
const normalize_decimal = (value: string) => {
  ...
  const magnitude = normalized_fraction
    ? `${normalized_integer}.${normalized_fraction}`
    : normalized_integer;
  return negative && magnitude !== "0" ? `-${magnitude}` : magnitude;
};

export const normalize_audit_scalar = (
...
const normalize_state = (
  ...
  return {
    state: "value",
    value: normalize_audit_scalar(value_type, state.value),
  };
};
...
const assert_identity = (input: AuditChangeIdentity) => {
  ...
  const parent_type_present =
    input.parent_entity_type !== undefined && input.parent_entity_type !== null;
  const parent_id_present =
    input.parent_entity_id !== undefined && input.parent_entity_id !== null;
  if (parent_type_present !== parent_id_present) {
    throw new AuditDomainError("invalid_audit_item");
  }
  if (input.parent_entity_type)
    assert_bounded(input.parent_entity_type, 80, "invalid_audit_item");
  if (input.parent_entity_id)
    assert_bounded(input.parent_entity_id, 26, "invalid_audit_item");
  if (input.logical_key)
    assert_bounded(input.logical_key, 255, "invalid_audit_item");
};

const states_equal = (left: AuditStateValue, right: AuditStateValue) =>
  left.state === right.state &&
  (left.state !== "value" ||
    (right.state === "value" && left.value === right.value));
...
type RowChangeInput = AuditChangeIdentity & {
...
type RedactedChangeInput = AuditChangeIdentity & {
  operation: "update";
  field_name: string;
};

export const create_redacted_change = (
  input: RedactedChangeInput,
): AuditChangeItem => {
  assert_identity(input);
...
export const create_scalar_change = (
  input: ScalarChangeInput,
): AuditChangeItem => {
  assert_identity(input);
  assert_bounded(input.field_name, 160, "invalid_audit_item");
  if (!operations.has(input.operation))
    throw new AuditDomainError("invalid_audit_operation");
  if (!value_types.has(input.value_type))
    throw new AuditDomainError("invalid_audit_value");
...
const validate_audit_item = (item: AuditChangeItem): AuditChangeItem => {
  ...
  if (
    ...
    item.before.state === "redacted" &&
    item.after.state === "redacted"
  ) {
    return create_redacted_change(item as RedactedChangeInput);
  }
...
export const validate_audit_event = (input: AuditEvent): AuditEvent => {
  ...
  if (input.actor_org_user_id)
    assert_bounded(input.actor_org_user_id, 26, "invalid_audit_actor");
  ...
  if (!source_types.has(input.source_type))
    throw new AuditDomainError("invalid_audit_source");
  if (!actor_types.has(input.actor_type))
    throw new AuditDomainError("invalid_audit_actor");
  if (input.outcome !== "committed")
    throw new AuditDomainError("invalid_audit_outcome");
  if (
    input.idempotency_key_hash &&
    !/^[0-9a-f]{64}$/u.test(input.idempotency_key_hash)
  ) {
  ...
  if (
    input.actor_type === "org_user"
      ...
      : Boolean(input.actor_org_user_id)
  ) {
    throw new AuditDomainError("invalid_audit_actor");
  }
  ...
  if (
    input.items.some(
      (item) =>
        item.organization_id !== input.organization_id ||
        item.audit_event_id !== input.id,
    )
  ) {
  ...
  if (
    ...
    (!Number.isSafeInteger(input.before_row_version) ||
      input.before_row_version < 0)
  ) {
    throw new AuditDomainError("invalid_audit_row_version");
  }
  if (
    input.after_row_version !== null &&
    (!Number.isSafeInteger(input.after_row_version) ||
      input.after_row_version < 0)
  ) {
    throw new AuditDomainError("invalid_audit_row_version");
  }
  if (
  ...
  if (
    Number.isNaN(occurred_at.valueOf()) ||
    occurred_at.toISOString() !== input.occurred_at
  ) {
