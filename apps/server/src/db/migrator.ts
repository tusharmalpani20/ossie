...
const migrations_path = path.join(path.dirname(fileURLToPath(import.meta.url)), "migrations");
...
export const run_migration_transaction = async (
  ...
  try {
    ...
    if (direction === "up") {
      await client.query("INSERT INTO db_migration.schema_migrations (name, executed_at) VALUES ($1, NOW())", [name]);
    } else {
      await client.query("DELETE FROM db_migration.schema_migrations WHERE name = $1", [name]);
    }
...
export const create_migrator = (pool: Pool, roles: { runtime_role: string; maintenance_role: string }) => {
  const storage = {
    async executed() {
    ...
    migrations: {
      ...
      resolve: (params: MigrationParams<{ pool: Pool }>) => {
        ...
        const run = async (direction: "up" | "down") => {
          ...
          if (!sql) throw new Error(`No ${direction.toUpperCase()} section found in migration ${name}`);
...
export const get_migrator = () => {
  ...
  if (!runtime_role || !maintenance_role) throw new Error("Database role identifiers must be defined");
  return { pool, umzug: create_migrator(pool, { runtime_role, maintenance_role }) };
};
