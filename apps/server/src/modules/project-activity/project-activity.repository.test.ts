...
import {
  build_project_activity_repository,
...
describe("Project Activity repository", () => {
  it("curates all Project Version lifecycle actions as Project activity", () => {
    expect(PROJECT_ACTIVITY_ACTIONS).toEqual(
  ...
  it("curates Revision, Carry-Forward, and completed Asset lifecycle actions", () => {
    expect(PROJECT_ACTIVITY_ACTIONS).toEqual(
      expect.arrayContaining([
  ...
  it("queries only the curated Audit allowlist in the requested tenant and Project", async () => {
    ...
    const result = await build_project_activity_repository({
      query,
    }).list_events({
    ...
    expect(query).toHaveBeenCalledWith(
      expect.stringContaining("FROM audit_schema.audit_event"),
      ["org-1", "project-1", PROJECT_ACTIVITY_ACTIONS, null, null, 21],
    );
