...
describe("Audit Evidence core", () => {
  ...
  it("enforces runtime append-only privileges and the Project guard", async () => {
    ...
    await pool.query("SELECT set_config('ossie.maintenance_mode', 'on', false)");
    await expect(
  ...
  it("rejects mismatched typed values, cross-tenant actors, and destructive Project deletion", async () => {
    ...
    await with_maintenance_client(async (client) => {
      await expect(client.query(
        "DELETE FROM project_schema.project WHERE id = $1",
        [project_id],
      )).rejects.toMatchObject({ code: "23503" });
    });
  });
  ...
  it("rejects invalid scalar operation transitions", async () => {
    ...
    `, [ulid(), event.rows[0].organization_id, event.rows[0].id, response.json().project.id]))
      .rejects.toMatchObject({ code: "23514" });
  });
  ...
  it("round-trips decimal, date, and timestamp values through typed columns", async () => {
    ...
    await write_audit_event(pool, {
      ...
      items: [
        ...
        create_scalar_change({
          id: ulid(), ...identity, operation: "create", field_name: "decimal",
          value_type: "decimal", before: { state: "absent" }, after: { state: "value", value: "12.340" },
        }),
        create_scalar_change({
          id: ulid(), ...identity, operation: "create", field_name: "date",
          value_type: "date", before: { state: "absent" }, after: { state: "value", value: "2026-07-19" },
        }),
        create_scalar_change({
          id: ulid(), ...identity, operation: "create", field_name: "timestamp",
          value_type: "timestamp", before: { state: "absent" }, after: { state: "value", value: "2026-07-19T01:02:03.000Z" },
        }),
      ],
    });
    ...
    expect(values.rows).toEqual([
      ...
      expect.objectContaining({ field_name: "decimal", decimal_value: "12.34" }),
  ...
  it("rolls back a context-bearing Project INSERT when deferred evidence is absent", async () => {
    ...
    try {
      ...
      await client.query("SELECT set_config('ossie.audit_event_id', $1, true)", [ulid()]);
      await client.query("SELECT set_config('ossie.audit_organization_id', $1, true)", [actor.rows[0].organization_id]);
      await client.query("SELECT set_config('ossie.audit_action', 'project.created', true)");
      await client.query("SELECT set_config('ossie.audit_command', 'project.create', true)");
      await client.query(`
        INSERT INTO project_schema.project (id, organization_id, name, created_by_id, updated_by_id)
