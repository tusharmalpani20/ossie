...
describe("Project compliance service", () => {
  it("authorizes Project compliance and forces the path Project scope", async () => {
    ...
    expect(access.authorize).toHaveBeenCalledWith(
      expect.objectContaining({ capability: "project.compliance.read" }),
    );
    expect(repository.list_events).toHaveBeenCalledWith(
      expect.objectContaining({ project_id: "01J00000000000000000000009" }),
    );
  });
});
...
describe("compliance service", () => {
  ...
  it("binds opaque cursors to the current filters", async () => {
    ...
    const first = await service.list_events({
      auth: auth(),
      query: { kind: "access", activity: "important", limit: 1 },
    });

    expect(repository.list_events).toHaveBeenCalledWith(
      expect.objectContaining({ activity: "important" }),
    );
  ...
  it.each([
    ...
    "rejects malformed cursor transport and payload before querying",
    async (cursor) => {
      const repository = {
        list_events: vi.fn(),
        get_audit_event_detail: vi.fn(),
