...
import type { GuideBlock, GuideDetail, GuideWorkingDraft } from "./guide.service";
...
export const build_audited_guide_screenshot_upload_service = (
  ...
  async upload(input: GuideScreenshotUploadInput): Promise<{ capture_asset: CaptureAsset; guide_block: GuideBlock; working_draft: GuideWorkingDraft }> {
    let guide_block: GuideBlock | null = null;
    let working_draft: GuideWorkingDraft | null = null;
      ...
      async transaction(callback) {
        ...
        const result = await run_audited_mutation({
          ...
          context: async (client) => {
            await client.query("SELECT id FROM guide_schema.guide_edition WHERE guide_id=$1 AND project_id=$2 AND organization_id=$3 AND project_version_id=$4 FOR UPDATE", [input.guide_id, input.project_id, input.auth.organization_id, input.project_version_id]);
            const guide_repository = build_guide_repository(client);
            before = await guide_repository.find_guide_detail({ organization_id: input.auth.organization_id, project_id: input.project_id, guide_id: input.guide_id, project_version_id: input.project_version_id });
          ...
          execute: async (client) => {
            ...
            const guide_repository = build_guide_repository(client);
            const updated = await guide_repository.update_guide_block_screenshot({
            ...
            guide_block = updated.guide_block;
            working_draft = updated.working_draft;
            after = await guide_repository.find_guide_detail({ organization_id: input.auth.organization_id, project_id: input.project_id, guide_id: input.guide_id, project_version_id: input.project_version_id });
            return asset;
          },
          build_event: (asset) => {
    ...
    return { capture_asset, guide_block: guide_block!, working_draft: working_draft! };
  },
});
