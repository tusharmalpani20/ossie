...
import { load_revision_snapshot } from "../documentation/documentation.repository";
import {
...
const access_candidate_sql = `
  SELECT actor.id org_user_id,actor_user.display_name,
         lower(actor_user.display_name) sort_value,
...
export const build_documentation_review_repository = (database: Database) => {
  const assert_review_writable = async (
    input: Scope,
    ...
    const state = await db.query<{
      ...
      `SELECT version.status project_version_status,
              edition.status edition_status
         FROM project_schema.project_version version
         JOIN documentation_schema.site_edition edition
           ON edition.project_version_id=version.id
          AND edition.organization_id=version.organization_id
          AND edition.project_id=version.project_id
        WHERE version.organization_id=$1 AND version.project_id=$2
          AND version.id=$3 AND edition.documentation_site_id=$4`,
      [
        input.organization_id,
        input.project_id,
    ...
    if (
      state.rows[0]?.project_version_status !== "active" ||
      state.rows[0]?.edition_status !== "active"
    )
      throw new DocumentationReviewError(
        "documentation_read_only",
        "Archived Documentation review state is read-only",
      );
  };

  const get_policy = async (input: Scope, db: Queryable = database) => {
    const result = await db.query<Record<string, any>>(policy_projection, [
      input.organization_id,
      input.project_id,
      input.project_version_id,
      input.site_id,
  ...
  const preview_gate = async (
    input: Scope & {
      revision_id: string;
      actor_can_override?: boolean;
    },
    db: Queryable = database,
  ) => {
    const policy = await get_policy(input, db);
    const request = await db.query<Record<string, any>>(
      `SELECT request.id,request.status,
         ...
         LEFT JOIN organization_schema.org_user reviewer
          ...
          AND reviewer.organization_id=request.organization_id
         LEFT JOIN project_schema.project_membership membership
           ON membership.organization_id=request.organization_id
          AND membership.project_id=request.project_id
          AND membership.org_user_id=reviewer.id
          AND membership.status='active'
         LEFT JOIN documentation_schema.documentation_review_maintainer maintainer
           ON maintainer.review_policy_id=$6
          AND maintainer.organization_id=request.organization_id
          AND maintainer.project_id=request.project_id
    ...
    const outcome = preview_documentation_review_gate({
      ...
      governing_request_status: governing?.status ?? null,
    });
    return {
      ...
      override_available_to_actor:
        ...
        outcome !== "approved" &&
        Boolean(input.actor_can_override),
    };
  };
  ...
  const get_request = async (
    ...
    const assignments = await db.query<Record<string, any>>(
      `SELECT assignment.id,assignment.reviewer_org_user_id,
              ...
              CASE WHEN reviewer.status='active'
                     AND (reviewer.role='owner' OR membership.status='active')
                   THEN CASE WHEN reviewer.role='owner'
                     THEN 'project_admin' ELSE membership.role END
                   ELSE NULL END current_project_role,
         ...
         LEFT JOIN project_schema.project_membership membership
           ON membership.organization_id=reviewer.organization_id
    ...
    const effective_status =
      policy.mode === "optional"
        ? request.status
        : documentation_review_effective_status(request.status, valid);
    const baselineResult = await db.query<{
      ...
      `SELECT id,revision_number
        ...
        WHERE site_edition_id=$1 AND revision_number<$2
        ORDER BY revision_number DESC LIMIT 1`,
      [request.site_edition_id, request.revision_number],
    );
    const [targetSnapshot, baselineSnapshot, publicationGate] =
      await Promise.all([
        load_revision_snapshot(db, {
          site_revision_id: request.site_revision_id,
          organization_id: input.organization_id,
          project_id: input.project_id,
          project_version_id: input.project_version_id,
          site_id: input.site_id,
        }),
        baselineResult.rows[0]
          ? load_revision_snapshot(db, {
              site_revision_id: baselineResult.rows[0].id,
              organization_id: input.organization_id,
              project_id: input.project_id,
              project_version_id: input.project_version_id,
              site_id: input.site_id,
        ...
        preview_gate(
          {
            ...input,
            revision_id: request.site_revision_id,
            actor_can_override: input.actor_is_admin,
          },
          db,
        ),
      ]);
    if (!targetSnapshot)
      throw new DocumentationReviewError(
        "documentation_review_request_not_found",
        "Review Request target was not found",
      );
    return {
      review_request: {
      ...
      actor_can_cancel:
        ...
        (request.created_by_id === input.actor_org_user_id ||
          Boolean(input.actor_is_admin)),
      change_summary: summarize_documentation_revision_snapshots(
        targetSnapshot,
        baselineSnapshot,
      ),
      ...
      cancellation:
        request.status === "canceled"
          ? {
              ...
              canceled_at:
                request.canceled_at instanceof Date
                  ? request.canceled_at.toISOString()
                  : request.canceled_at,
  ...
  return {
    ...
    update_policy: (
      ...
      transaction(database, async (client) => {
        ...
        await assert_review_writable(input, client);
    ...
    list_candidates: async (
      ...
      await assert_review_writable(input);
      const policy = await get_policy(input);
      const limit = input.limit ?? 50;
      const cursorContext = {
        family: "candidates" as const,
        scope: {
          organization_id: input.organization_id,
          project_id: input.project_id,
          project_version_id: input.project_version_id,
      ...
      const cursor = input.cursor
        ? decode_documentation_review_cursor(input.cursor, cursorContext)
        : null;
      const result = await database.query<Record<string, any>>(
        `${access_candidate_sql}
          AND ($3::text IS NULL OR
            (lower(actor_user.display_name),actor.id) > ($3,$4))
          ORDER BY lower(actor_user.display_name),actor.id LIMIT $5`,
        [
          input.organization_id,
          input.project_id,
      ...
      return {
        candidates: rows.map((row) => ({
          ...
          is_organization_owner: row.is_organization_owner,
          ...
          result.rows.length > limit && last
            ? encode_documentation_review_cursor({
                ...
                position: {
                  sort_value:
                    last.sort_value ??
                    String(last.display_name).toLocaleLowerCase("en-US"),
                  id: last.org_user_id,
                },
              })
            : null,
    ...
    list_requests: async (
      ...
      const limit = input.limit ?? 50;
      const selectedStatus = input.status ?? "open";
      const selectedParticipation = input.participation ?? "all";
      const cursorContext = {
      ...
      const result = await database.query<Record<string, any>>(
        `WITH projected AS (
          SELECT request.*,revision.revision_number,
                 ...
                 CASE WHEN request.status='approved'
                   AND policy.mode='approval_required'
                   AND NOT (
                     count(decision.id) FILTER (
                       WHERE decision.decision='approve'
                         AND reviewer.status='active'
                     ...
                     AND (
                       NOT policy.require_maintainer_approval OR
                       count(decision.id) FILTER (
                         WHERE decision.decision='approve'
                           AND reviewer.status='active'
                           AND (reviewer.role='owner' OR membership.id IS NOT NULL)
                   ...
                   ) THEN 'invalidated' ELSE request.status
            ...
            JOIN organization_schema.org_user creator
              ON creator.id=request.created_by_id
             AND creator.organization_id=request.organization_id
            JOIN user_schema.user creator_user ON creator_user.id=creator.user_id
            JOIN documentation_schema.documentation_review_policy policy
              ON policy.site_edition_id=request.site_edition_id
             AND policy.organization_id=request.organization_id
             AND policy.project_id=request.project_id
            LEFT JOIN documentation_schema.documentation_review_assignment assignment
              ON assignment.review_request_id=request.id
            LEFT JOIN documentation_schema.documentation_review_decision decision
              ON decision.review_assignment_id=assignment.id
           ...
           WHERE request.organization_id=$1 AND request.project_id=$2
             ...
             AND (
               $7::varchar='all'
               OR ($7::varchar='requested_by_me' AND request.created_by_id=$5)
               OR ($7::varchar='assigned_to_me' AND EXISTS (
                 SELECT 1
                   FROM documentation_schema.documentation_review_assignment mine
                  WHERE mine.review_request_id=request.id
                    AND mine.reviewer_org_user_id=$5
               ))
             )
           GROUP BY request.id,revision.revision_number,
                    creator_user.display_name,policy.id
        )
        SELECT * FROM projected
         WHERE ($6::varchar='all' OR effective_status=$6::varchar)
           AND ($8::timestamptz IS NULL OR
             (created_at,id) < ($8::timestamptz,$9::varchar))
      ...
      const last = rows.at(-1);
      return {
        review_requests: rows.map((request) =>
          to_review_request(request, request.effective_status, {
            approvals: request.valid_approval_count,
            maintainers: request.valid_maintainer_approval_count,
          }),
        ),
        next_cursor:
          result.rows.length > limit && last
        ...
        if (status === "open") {
          const count = await client.query<{
            ...
            `SELECT count(*)::int approvals,
               ...
               JOIN organization_schema.org_user reviewer
                 ON reviewer.id=assignment.reviewer_org_user_id
                AND reviewer.organization_id=decision.organization_id
                AND reviewer.status='active'
               LEFT JOIN project_schema.project_membership membership
    ...
    list_inbox: async (
      ...
      const limit = input.limit ?? 50;
      const selectedStatus = input.status ?? "unread";
      const cursorContext = {
        family: "inbox" as const,
        scope: {
          organization_id: input.organization_id,
          project_id: input.project_id,
          project_version_id: input.project_version_id,
      ...
      const result = await database.query<Record<string, any>>(
        `SELECT notification.*,
          ...
          WHERE notification.organization_id=$1
            ...
            AND ($5::varchar='all' OR notification.status=$5::varchar)
            AND ($6::timestamptz IS NULL OR
              (notification.created_at,notification.id) <
                ($6::timestamptz,$7::varchar))
          ORDER BY notification.created_at DESC,notification.id DESC LIMIT $8`,
        [
          input.organization_id,
          input.project_id,
          input.project_version_id,
      ...
      const last = rows.at(-1);
      return {
        items: rows.map(({ display_context, ...notification }) => ({
          notification: {
            ...notification,
    ...
    list_evidence: async (
      ...
      const limit = input.limit ?? 50;
      const selectedOutcome = input.outcome ?? "all";
      const cursorContext = {
        ...
        filters: {
          ...
          site_publication_id: input.site_publication_id ?? null,
      ...
      const result = await database.query<Record<string, any>>(
        `SELECT id,site_revision_id,site_publication_id,publish_link_id,
          ...
          WHERE organization_id=$1 AND project_id=$2
            AND project_version_id=$3 AND documentation_site_id=$4
            AND ($5::varchar IS NULL OR site_revision_id=$5)
            AND ($6::varchar IS NULL OR site_publication_id=$6)
            AND ($7::varchar='all' OR outcome=$7)
            AND ($8::timestamptz IS NULL OR
              (created_at,id) < ($8::timestamptz,$9::varchar))
          ORDER BY created_at DESC,id DESC LIMIT $10`,
        [
          ...
          input.site_publication_id ?? null,
          selectedOutcome,
          cursor?.sort_value ?? null,
          cursor?.id ?? null,
          limit + 1,
