...
const owner = { org_user_id: "owner-1", email: "owner@example.test", display_name: "Owner", organization_role: "owner", organization_status: "active", access_source: "organization_owner", membership: null, effective_project_role: "project_admin" };
const candidate = { org_user_id: "member-1", email: "member@example.test", display_name: "Member", organization_role: "member", organization_status: "active", access_source: null, membership: null, effective_project_role: null };
...
describe("ProjectMembershipSection", () => {
  it("shows immutable owners and assigns an active unassigned member", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Organization member"), { target: { value: "member-1" } });
    fireEvent.click(screen.getByRole("button", { name: "Assign access" }));
    await waitFor(() => expect(fetch).toHaveBeenCalledWith("/api/v1/projects/project-1/memberships", expect.objectContaining({ method: "POST" })));
    expect(await screen.findByText("Project access assigned.")).toBeInTheDocument();
  });
});
