...
describe("ComplianceTimelinePage", () => {
  ...
  it("ignores a stale pagination response after the evidence filter changes", async () => {
    const pagedPage: ComplianceEventsResponse = {
      ...accessPage,
    ...
    let resolveMore!: (response: ComplianceEventsResponse) => void;
    let resolveAudit!: (response: ComplianceEventsResponse) => void;
    const loadEvents = vi.fn(
      (
        ...
        if ((input.kind ?? "all") === "all" && !input.cursor)
          return Promise.resolve(pagedPage);
        if (input.cursor)
          return new Promise<ComplianceEventsResponse>((resolve) => {
            resolveMore = resolve;
          });
        return new Promise<ComplianceEventsResponse>((resolve) => {
    ...
    await waitFor(() => expect(resolveAudit).toBeTypeOf("function"));
    resolveAudit(auditPage);
    expect(await screen.findByText("audit.changed")).toBeInTheDocument();
