...
describe("ComplianceTimelinePage", () => {
  it("renders retained totals and reloads when the evidence kind changes", async () => {
    ...
    render(<ComplianceTimelinePage loadEvents={loadEvents} performLogout={vi.fn()} />);
    ...
    fireEvent.change(screen.getByLabelText("Evidence kind"), { target: { value: "audit" } });
    await waitFor(() => expect(loadEvents).toHaveBeenLastCalledWith({ kind: "audit" }));
    expect(screen.getByRole("link", { name: "Organization members" })).toHaveAttribute("href", "/organization/members");
  });
  ...
  it("renders an Owner-required state without evidence", async () => {
    ...
    expect(await screen.findByText("Only organization owners can view compliance evidence.")).toBeInTheDocument();
